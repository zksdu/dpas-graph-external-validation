...
class GATv2Conv(nn.Module):
    """Manual single-head GATv2 (gather + index_add only).

    Drop-in replacement for torch_geometric.nn.GATv2Conv with
    heads=1, concat=True, add_self_loops=False. Avoids MessagePassing
    scatter/gather codegen paths that segfault on this machine.
    """

    def __init__(self, in_channels, out_channels, heads=1, concat=True,
                 edge_dim=2, add_self_loops=False, bias=True, **kwargs):
        super().__init__()
        assert heads == 1 and concat
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        if edge_attr is not None and self.lin_edge is not None:
            ...
            h = h + self.lin_edge(edge_attr)
        h = F.leaky_relu(h, self.negative_slope)
        e = (h * self.att).sum(-1).clamp(-30, 30)
        ex = e.exp()
        denom = torch.zeros(x.size(0), dtype=ex.dtype,
                            device=ex.device).index_add_(0, dst, ex)
        alpha = ex / denom[dst].clamp_min(1e-16)
        out = torch.zeros(x.size(0), xl.size(1), dtype=xl.dtype,
                          device=xl.device).index_add_(
            0, dst, xl[src] * alpha.unsqueeze(-1))
...
class DualAdaptiveEncoder(nn.Module):
    ...
    def _encode_layer(self, l, h, edge_index, w_sp0, w_ft0):
        """Single encoder layer (checkpoint-friendly: no side outputs)."""
    ...
    def forward(self, x, edge_index, edge_weight=None, return_debug: bool = False):
        ...
        for l in range(len(self.conv_sp)):
            # Checkpointed path: recompute layer activations during backward,
            # keeping only layer inputs alive. Avoids OOM on large spots x dims.
            if self.use_checkpoint and self.training and not return_debug:
                import torch.utils.checkpoint as _cp
                h = _cp.checkpoint(
                    self._encode_layer, l, h, edge_index, w_sp0, w_ft0,
                    use_reentrant=False)
                continue
