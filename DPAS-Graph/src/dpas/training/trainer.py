...
def _load_state_bundle(load_dir, prefix, encoder_mRNA, decoder_mRNA, decoder_protein, device):
    encoder_mRNA.load_state_dict(torch.load(os.path.join(load_dir, f"{prefix}_encoder_mRNA.pth"), map_location=device, weights_only=False))
    decoder_mRNA.load_state_dict(torch.load(os.path.join(load_dir, f"{prefix}_decoder_mRNA.pth"), map_location=device, weights_only=False))
    decoder_protein.load_state_dict(torch.load(os.path.join(load_dir, f"{prefix}_decoder_protein.pth"), map_location=device, weights_only=False))
...
def train_and_evaluate_fold(
    ...
    if init_ckpt_dir is not None:
        ...
        if init_ckpt_tag == "legacy":
            encoder_mRNA.load_state_dict(torch.load(os.path.join(init_ckpt_dir, "encoder_mRNA.pth"), map_location=device, weights_only=False))
            decoder_mRNA.load_state_dict(torch.load(os.path.join(init_ckpt_dir, "decoder_mRNA.pth"), map_location=device, weights_only=False))
            decoder_protein.load_state_dict(torch.load(os.path.join(init_ckpt_dir, "decoder_protein.pth"), map_location=device, weights_only=False))
        else:
...
def protein_predict(model_save_dir, adata, common_gene, common_protein, ckpt_prefix: str = "last"):
    ...
    enc.load_state_dict(torch.load(enc_path, map_location=device, weights_only=False))
    dec.load_state_dict(torch.load(dec_path, map_location=device, weights_only=False))
