# -*- coding: utf-8 -*-
"""Fig3（架构对比）与 Fig4（机制与对照）——全部数字从存档结果表实时计算，无硬编码统计值。

数据源：
- Fig3A/B: results/competitor_benchmark_per_protein.csv (ridge_pc, 4 外部样本) +
           server_round2/dgat_lodo_gse_per_protein.csv (DGAT, 2 折 × 4 样本)
- Fig3C:   DGAT 自带 700 基因 spot 过滤后的 n_spots（dgat 表）vs 总 spot 数（internal_vs_external_summary.csv）
- Fig3D:   results/lodo100ep_multiseed_summary.csv (3 seeds)
- Fig4A:   results/designC_marker_importance.csv (31 标志物消融)
- Fig4B:   results/rna_coverage_diagnosis.csv (两数据集)
- Fig4C:   results/visium_eope_score.csv（或 _A/_B）逐 spot 签名分 → Mann-Whitney AUC(A>B)
- Fig4D:   results/internal_vs_external_summary.csv（6 样本模型 SP + 同型地板）
...
# A: ridge vs DGAT per-protein scatter
ax = axs[0, 0]
for p in markers:
...
ax.set_title("A  Per-protein: DGAT vs ridge (31 markers)", fontsize=8.5, loc="left")
...
# B: paired difference DGAT - ridge
ax = axs[0, 1]
...
nwin = int((diff > 0).sum())
ax.set_title(f"B  DGAT wins {nwin} of 31 markers vs ridge", fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)

# C: DGAT spot retention
ax = axs[1, 0]
...
ax.set_ylim(0, 112)
ax.set_title("C  DGAT spot filter: retention per section", fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)

# D: 3-seed stability at 100 epochs
ax = axs[1, 1]
...
ax.set_ylim(0, 0.45)
ax.set_title(f"D  Full-budget stability: {m:.3f} ± {sd:.3f} across seeds",
             fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)

plt.tight_layout(rect=[0, 0, 1, 0.97])
plt.savefig("results/fig3_architecture.png", dpi=600)
plt.close(fig)
print(f"fig3 saved | ridge={ridge_mean:.4f} DGAT={dgat_m[markers].mean():.4f} "
...
ax.set_title("A  Per-marker ablation (31 retrainings)",
             fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)
...
ax.set_title("B  RNA coverage of namesake genes",
             fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)

# C: section identity AUC (recomputed from per-spot signature scores)
ax = axs[1, 0]
...
for s in sigs:
    ...
    u = len(xa) * len(xb) * 0.5  # placeholder, real below
...
ax.set_title("C  Section identity unresolved: all AUCs ≈ 0.5", fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)

# D: internal vs external per sample
ax = axs[1, 1]
iv = tot.copy()
...
ax.set_title("D  Internal LODO vs external sections",
             fontsize=8.5, loc="left")
ax.spines[["top", "right"]].set_visible(False)

plt.tight_layout(rect=[0, 0, 1, 0.97])
plt.savefig("results/fig4_mechanism_controls.png", dpi=600)
plt.close(fig)
print(f"fig4 saved | AUCs {({k: round(v,3) for k,v in aucs.items()})} "
