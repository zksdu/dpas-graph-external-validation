#!/bin/bash
# Zenodo 大文件多连接分段并行下载器
# 用法: zenodo_parallel.sh <file_key> <total_bytes> <n_parts> <outfile>
KEY="$1"; TOTAL="$2"; N="$3"; OUT="$4"
URL="$KEY"
DIR="${OUT}.parts"
mkdir -p "$DIR"
CHUNK=$(( (TOTAL + N - 1) / N ))
for i in $(seq 0 $((N-1))); do
  START=$((i * CHUNK))
  END=$((START + CHUNK - 1))
  if [ $END -ge $TOTAL ]; then END=$((TOTAL - 1)); fi
  if [ $START -ge $TOTAL ]; then break; fi
  PART="$DIR/part_$(printf '%03d' $i)"
  (
    expected=$((END - START + 1))
    for attempt in $(seq 1 40); do
      have=0
      [ -f "$PART" ] && have=$(stat -c %s "$PART")
      if [ "$have" -ge "$expected" ]; then break; fi
      s=$((START + have))
      need=$((expected - have))
      tmp="$PART.tmp.$$"
      # 严格模式：先写临时文件，只在大小精确匹配剩余需求时才追加，避免重复/半截数据
      curl -sS --ssl-no-revoke -L --retry 2 --retry-delay 5 --speed-limit 2000 --speed-time 300 \
        -r ${s}-${END} -o "$tmp" "$URL" 2>/dev/null
      got=0
      [ -f "$tmp" ] && got=$(stat -c %s "$tmp")
      # 规则：got<=need 视为有效（含部分数据，累积进度）；got>need 说明服务端忽略 Range，判为重复丢弃
      if [ "$got" -gt 0 ] && [ "$got" -le "$need" ]; then
        cat "$tmp" >> "$PART"
        rm -f "$tmp"
      else
        rm -f "$tmp"
        echo "part_$(printf '%03d' $i) retry (got=$got need=$need)" >> "$DIR/../download_warn.log"
      fi
    done
  ) &
done
wait
# 合并
> "$OUT"
for i in $(seq 0 $((N-1))); do
  PART="$DIR/part_$(printf '%03d' $i)"
  [ -f "$PART" ] && cat "$PART" >> "$OUT"
done
have=$(stat -c %s "$OUT" 2>/dev/null || echo 0)
echo "downloaded=$have expected=$TOTAL"
if [ "$have" = "$TOTAL" ]; then
  rm -rf "$DIR"
  echo "OK"
else
  echo "INCOMPLETE"
fi
